...
class _BaseballMatchDetailScreenState extends ConsumerState<BaseballMatchDetailScreen> {
  Timer? _eventsTimer;
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (!mounted) return;
      ref.read(baseballRealtimeProvider.notifier).setWatchedIds('detail:${widget.matchId}', [
        widget.matchId,
      ]);
    });
    _eventsTimer = Timer.periodic(const Duration(seconds: 5), (_) {
      if (!mounted) return;
      ref.invalidate(baseballMatchEventsProvider(matchId: widget.matchId));
    });
  }
  ...
  void dispose() {
    _eventsTimer?.cancel();
    ref.read(baseballRealtimeProvider.notifier).clearSource('detail:${widget.matchId}');
    super.dispose();
  }

  @override
  Widget build(BuildContext context) {
    final detailAsync = ref.watch(baseballMatchDetailProvider(matchId: widget.matchId));

    ref.listen(baseballRealtimeProvider.select((map) => map[widget.matchId]?.statusId), (
      prev,
      next,
    ) {
      if (prev == null || next == null || prev == next) return;
      ref.invalidate(baseballMatchDetailProvider(matchId: widget.matchId));
      ref.invalidate(baseballMatchEventsProvider(matchId: widget.matchId));
    });
    ...
    return DefaultTabController(
      ...
      child: Scaffold(
        ...
        body: Column(
          children: [
            _BaseballMatchHeader(matchId: widget.matchId),
            Container(
              color: Colors.white,
              child: TabBar(
...
class _BaseballMatchHeader extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final rt = ref.watch(baseballRealtimeProvider.select((map) => map[matchId]));
